...
class Envol {
public:
	Envol(const Paciente& p, const Lista<CodigoPaciente>::Iterator &i, Gravedad &g);
	const Paciente& p() const;
	const Lista<CodigoPaciente>::Iterator& i() const;
	const Gravedad& g() const;
private:
	Paciente _p;
